...
//       // Initialize attendance records with existing data if available
...
//   //   // send QR payload to backend scan endpoint (backend will resolve student/branch/class)
//   //   try {
//   //     const res = await apiClient.post(API_ENDPOINTS.BRANCH_ADMIN.ATTENDANCE.SCAN, {
//   //       qr: qrData,
//   //       date: attendanceDate,
...
//   //       if (student) {
//   //         // update local UI if student is in current list
//   //         if (student._id && students.find(s => s._id === student._id)) {
...
//     if (!qrData) {
//       toast.error('No QR data received');
//       return;
//     }

//     // Extract the actual text from QR
//     let qrText = '';
...
//     console.log('QR Text to send:', qrText);

//     try {
//       const payload = {
//         qr: qrText,  // Send just the text, not the whole object
//         date: attendanceDate,
//         subjectId: selectedSubject || null,
//         attendanceType: selectedSubject ? 'subject' : 'daily'
//       };
...
//       let errorMsg = 'Failed to record scan';
...
//   // if (loading && !classes.length) return <FullPageLoader />;
...
export default function BranchAdminAttendancePage() {
  ...
  const handleQRScan = async (qrData) => {
    ...
    const payload = {
      qr: qrData,  // Send the object directly
      date: attendanceDate,
      subjectId: selectedSubject || null,
      attendanceType: selectedSubject ? 'subject' : 'daily'
    };
