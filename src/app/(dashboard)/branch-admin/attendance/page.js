...
export default function AttendancePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiClient.post(API_ENDPOINTS.BRANCH_ADMIN.ATTENDANCE.CREATE, formData);
      if (response.success) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await apiClient.delete(API_ENDPOINTS.BRANCH_ADMIN.ATTENDANCE.DELETE.replace(':id', id));
      if (response.success) {
