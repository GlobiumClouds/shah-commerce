...
async function addFaculty() {
  ...
  try {
    const response = await axios.post(`${API_BASE_URL}/api/super-admin/faculties`, {
      ...
      code: 'MED',
      description: 'Faculty for Medical and Health Sciences'
    }, {
      headers: {
    ...
    if (response.data.success) {
      ...
      console.log('❌ Failed to add faculty:', response.data.message);
      return null;
    }
  } catch (error) {
    console.log('❌ Add faculty error:', error.response?.data?.message || error.message);
    return null;
  }
}
 ...
 */
async function main() {
  console.log('🚀 Super Admin Login and Faculty Addition Script');
  ...
  if (!faculty) {
    console.log('❌ Script failed: Could not add faculty');
    return;
  }
